...
- **VSync-Aligned Auto-Scrolling**: In `useAutoScroll`, always schedule layout queries and smooth scrolling via `requestAnimationFrame` with a cancellation cleanup ref (`rafRef`). Never use arbitrary `setTimeout` delays, and cancel pending frames on rapid cue transitions to prevent conflicting scroll triggers and layout thrashing.
