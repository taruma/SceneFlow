...
- **Forward Monotonic Scrolling Guard for Enclosing Intervals**: When auto-scrolling chronologically ordered media lists containing overlapping or hierarchical intervals (e.g. longer Action/Scene cues wrapping shorter nested Dialogue cues), enforce forward monotonic scrolling (`furthestScrollTopRef`). Target scroll positions must not jump backward during forward playback when a nested child cue finishes and leaves an earlier-starting enclosing cue as the active resolver. Automatically invalidate/reset the monotonic guard upon mode transition into edit mode (`prevMode !== 'edit' && mode === 'edit'`), backward seeks (`currentTime < prevTime - 0.3s`), forward scrubber jumps (`currentTime - prevTime > 1.5s`), manual cue selection (`selectedCueId`), category filter toggles, search query changes, or manual scroll adjustments. In inter-cue silence gaps where no cue is actively firing, auto-scroll targets the next upcoming cue (`findScrollTargetCue`) to keep viewport positioning anchored to upcoming scene text without false active glow styling.
...
- **Continuous Clock Extrapolation Over CSS Transitions**: Never bridge discrete, jittery external timecode (~100ms polling) with fixed-duration CSS transitions (`100ms linear`). Always extrapolate time continuously at the monitor's native refresh rate (60Hz, 120Hz, etc.) using `requestAnimationFrame` + `performance.now()` with soft drift compensation, halting immediately on pause for 0 CPU idle load.
...
- **Multi-Platform Screen-Recording Baseline**: Benchmark playback smoothness on desktop Windows Chromium across high-refresh monitors and in 60fps screen recordings; do not rely solely on mobile flagship hardware compositors to validate animation pacing.
