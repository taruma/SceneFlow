...
export const ScriptEditorCanvas = React.memo(function ScriptEditorCanvas({
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      {/* Editor Subheader Rail (Matches h-9 of Outline and Guide to lock the horizontal horizon) */}
      <div className="h-9 px-3.5 border-b border-border-subtle bg-surface-subtle/50 flex items-center justify-between shrink-0 select-none">
        <div className="flex items-center gap-2 text-text-muted">
          <FileText size={12} className="text-purple-500" />
          <span className="text-[10px] font-bold uppercase tracking-wider text-text-main">
            Script Canvas
          </span>
          <span className="text-[9px] font-mono text-text-faint px-1.5 py-0.2 rounded bg-surface border border-border-subtle/60">
            Fountain / Text
          </span>
        </div>

        <div className="flex items-center gap-2 text-[9px] font-mono text-text-faint">
      ...
      {/* Editor Body: Gutter + Hidden Mirror + Textarea */}
      <div 
        className={cn(
