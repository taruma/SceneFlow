...
1. **Update Line Types**: Add the new type identifier to the `LineType` union in [`src/lib/scriptProcessor.ts`](file:///e:/_github/Screenplay-Sync/src/lib/scriptProcessor.ts):
   ```typescript
   export type LineType = 
     ...
     | 'new-type';
   ```
2. **Implement Detection Logic**: Update `processScript()` in [`src/lib/scriptProcessor.ts`](file:///e:/_github/Screenplay-Sync/src/lib/scriptProcessor.ts) with regex or heuristic rules to classify the line into your new type.
3. **Map Visual Styles**: Update `getLineClass()` in [`src/lib/scriptStyles.ts`](file:///e:/_github/Screenplay-Sync/src/lib/scriptStyles.ts) to return the appropriate Tailwind CSS classes, referencing active theme tokens (e.g., `theme.textColor`, `theme.textMutedColor`).
4. **Block-Level Types (Optional)**: If the new type applies to the structured block parser used for export or AST representations, add it to `ScriptBlockType` in [`src/types/script.ts`](file:///e:/_github/Screenplay-Sync/src/types/script.ts) and update `parseScriptToBlocks()` in [`src/lib/scriptParser.ts`](file:///e:/_github/Screenplay-Sync/src/lib/scriptParser.ts).

## 2. Modifying Styles & Script Themes

**DO NOT** write hardcoded Tailwind color classes directly into [`src/App.tsx`](file:///e:/_github/Screenplay-Sync/src/App.tsx) for screenplay text or cue highlights.

- **Centralized Themes**: All paper surfaces, borders, shadows, punch holes, headings, staging badges, and brief cards are configured in `SCRIPT_THEMES` within [`src/lib/scriptStyles.ts`](file:///e:/_github/Screenplay-Sync/src/lib/scriptStyles.ts).
...
The `renderedScript` `useMemo` in [`src/App.tsx`](file:///e:/_github/Screenplay-Sync/src/App.tsx) is executed frequently as playback time updates:
...
- **State Schema**: Maintain the `AppState` interface in [`src/App.tsx`](file:///e:/_github/Screenplay-Sync/src/App.tsx) (`youtubeId`, `scriptText`, `cues: Cue[]`, `settings?: Record<string, TimingSettings>`).
- **LocalStorage Keys**:
...
- **Query Parameters**: On application mount, inspect `window.location.search`:
  - `?example=ID`: Matches an example `id` from `EXAMPLE_SECTIONS` in [`src/examples.ts`](file:///e:/_github/Screenplay-Sync/src/examples.ts).
...
- **Desktop vs. Mobile Modals**: Desktop browsing uses the full-featured [`src/components/LibraryModal.tsx`](file:///e:/_github/Screenplay-Sync/src/components/LibraryModal.tsx) with search, sorting, and category sidebar. Mobile devices use the touch-optimized bottom-sheet drawer [`src/components/MobileLibraryModal.tsx`](file:///e:/_github/Screenplay-Sync/src/components/MobileLibraryModal.tsx). Both share state and are mutually exclusive based on viewport width (`lg` breakpoint).
