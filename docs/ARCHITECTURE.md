...
14. **`ScriptHeaderControls.tsx`**: Playback-mode control bar with auto-scroll toggle, viewport-safe left-aligned target-type multi-select Focus Mode dropdown (`UI_TOKENS.dropdown.menu` with dynamic `getCueColorForTheme` category dots synchronized to active script theme and CVD profile), reading width preset selector, scroll focus preset selector (with auto-close dismissal on selection), non-blocking 1-click outside dismissal across all dropdowns via `useClickOutside` and `useEscapeKey`, compact mobile quick-access tools (Theme Palette, Library, and Support), and 0 Hz playback re-render decoupling via `React.memo`.
