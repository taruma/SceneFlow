...
15. **`ActiveHighlightsPanel` (`src/components/active-highlights/`)**: Modular playback visualization sub-package featuring:
    - **`ActiveHighlightsPanel.tsx`**: Main orchestrator featuring a balanced header layout with an anchored **Studio VU Meter** (8-slot category LED indicator strip with zero layout shift), jitter-free isolated numeric counter (`min-w-[14px] font-mono tabular-nums`), persistent timeline zoom presets (`[ 4s | 8s | 16s ]`) anchored before filters in timeline mode (`sceneflow_timeline_zoom_preset`), collapsible filter drawer (`sceneflow_highlight_filter_expanded`), dedicated `Filters` toggle button with active mute pip indicator, and persistent segmented view switcher (`[ 📊 Timeline | 🗂 Cards ]`).
    - **`HighlightTimelineView.tsx`**: Multi-Track Sync Timeline view with dynamic density scaling (`TimelineDensity`: `'comfortable'` 32px vs. `'compact'` 24px), timeline zoom preset integration (`zoomPreset`: `'4s' | '8s' | '16s'`), stationary 35% anticipation playhead, and docked inspector card.
    - **`HighlightCardsView.tsx`**: Classic floating cards presentation for legacy playback visualization.
    - **`useTimelineWindow.ts`**: Headless rolling window hook with global greedy interval scheduling for sub-lanes, adaptive timecode tick marks, and exposure of `scriptCategories`.
    - **`TimelineLane.tsx` & `TimelineCueBlock.tsx`**: Isolated track components with hardware-accelerated CSS transitions, theme coloring, synchronized density offsets, narrow block label elision (`widthPercent < 3.5%`), and interactive lane headers that toggle category visibility.
