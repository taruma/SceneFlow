...
14. **`ScriptHeaderControls.tsx`**: Playback-mode control bar with auto-scroll toggle, target-type multi-select Focus Mode dropdown (with dynamic `getCueColorForTheme` category dots synchronized to active script theme and CVD profile), reading width preset selector, and scroll focus preset selector.
