...
15. **`ActiveHighlightsPanel` (`src/components/active-highlights/`)**: Modular playback visualization sub-package featuring:
    - **`ActiveHighlightsPanel.tsx`**: Main orchestrator featuring a balanced header layout with an anchored **Studio VU Meter** (8-slot category LED indicator strip with zero layout shift), jitter-free isolated numeric counter (`min-w-[14px] font-mono tabular-nums`), persistent track height mode toggle (`[ Flex | Fixed ]`, `sceneflow_timeline_height_mode`), persistent timeline zoom presets (`[ 4s | 8s | 16s ]`, `sceneflow_timeline_zoom_preset`), collapsible filter drawer (`sceneflow_highlight_filter_expanded`), dedicated `Filters` toggle button with active mute pip indicator, and persistent segmented view switcher (`[ 📊 Timeline | 🗂 Cards ]`).
