...
1. **`AppHeader.tsx`**: Global 3-zone studio navigation header featuring brand identity and desktop document sync utilities (`[ 📂 | 💾 ]`) on the left, a centered segmented mode switcher (`[ ▶ Playback | ✏️ Edit ]`) with mode-specific active accents, and a right-wing studio cluster housing the standalone Library gateway (`[ 📚 Library ]`), Ko-fi support pill, unified Studio Preferences dropdown (`[ ⚙️ Settings ▾ ]` with 4-theme quick selector, script palette, timing, and layout reset), and dedicated Info modal trigger.
