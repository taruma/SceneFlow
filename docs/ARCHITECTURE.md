...
5. **`WorkstationLeftPanel.tsx` (`src/components/left-panel/`) & `SyncCuesPanel.tsx`**: Studio-grade Two-Tier Flex Left Panel for both Playback and Edit modes with adaptive container queries (`@container (max-width: 580px)`):
   - **Tier 1 (Persistent Media Viewport & Header)**: Permanently mounted media container hosting `MediaHeader.tsx` (transport controls `[Replay]`, `[Play/Pause]`, `[Hide/Show Video]`, live timecode HUD badge `LiveTimecodeBadge.tsx`, and collapsible YouTube source pill `[ 🟢 {videoId} ✏️ ]`), isolated memoized video viewport (`MediaViewport.tsx`) with static `YOUTUBE_PLAYER_OPTS` to eliminate iframe destruction and audio/playback resets across mode changes, and horizontal `VideoSplitDivider` (tightened `mt-2 mb-1`).
